...
`atem` (AutoTask EasyMode) is a small, stdlib-only Go CLI that wraps the Autotask
PSA REST API. It keeps loose local work timers and turns plain technician
workflows into tickets, time entries, and AI-friendly reports. It is designed to
be driven by a human **or** an AI agent from the terminal.
...
cmd_*.go           one file per command group (company/resource/ticket/timer/time/report/config/ui)
...
help`) is generated from `registry.go`, so it can never drift from the commands.
...
3. Register it in `commands` in `registry.go`: name, summary, flags (with
   ...
   and `OutputType` (+ `DryRunType` for a dry-run shape). This wires dispatch AND
   exposes it via `describe` + MCP. Keep the registry flags in sync with the
   handler's `newFlagSet` — `TestRegistryFlagsAreDefined` guards against drift.
