...
func (a *App) cmdTimeAdd(args []string) (*cmdResult, error) {
	...
	createTicket, err := a.timeAddTicket(*ticket, *company, *title, *desc, day)
	if err != nil {
		return nil, err
	}
...
func (a *App) timeAddTicket(ticket int64, company, title, desc string, day time.Time) (map[string]any, error) {
	switch {
	case ticket != 0:
		return nil, nil
	case strings.TrimSpace(company) != "":
		...
		fields, _ := a.ticketFields(companyID, t, desc)
		return fields, nil
	default:
